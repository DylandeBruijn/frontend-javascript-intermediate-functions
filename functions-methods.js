...
function getEmailDomain(emailadres) {
    let emailSubstrings = emailadres.split("@");
    return emailSubstrings[1];
}
...
function checkEmailValidity(emailadres) {
    let atSign = emailadres.includes("@");
    let noComma = !emailadres.includes(",");
    let noDot = emailadres.charAt(emailadres.length - 1);
    if (atSign && noComma && noDot !== ".") {
        return true;
    } else {
        return false;
    }
